 ...
 *
 * Handles messages from the dashboard (via content-bridge.js) and
 * orchestrates tab opening, login detection, and data extraction.
 *
 * v3 — Fixed Cloud Coach two-step navigation, aria-label parsing, frame prioritization,
 *      and readiness timeout handling.
 */
...
/** Find an existing tab whose URL starts with a given prefix. */
async function findTab(urlPrefix) {
  const tabs = await chrome.tabs.query({});
  return tabs.find((t) => t.url && t.url.startsWith(urlPrefix));
}

/** Open a URL in a new tab or find existing. Wait for load. */
async function ensureTab(urlPrefix, fullUrl) {
  let tab = await findTab(urlPrefix);
  if (tab) {
    // Focus existing tab
...
// These run inside the target page via chrome.scripting.executeScript.
// They must be fully self-contained (no external references).

// ═══════════════════════════════════════════════════════
// ── Salesforce CRM Analytics (Wave) Report ────────────
// ═══════════════════════════════════════════════════════

/**
 * Readiness check for Salesforce Analytics report.
 * Returns true when:
 * - div.report-table-widget exists with widgetReady AND finalState classes
...
function waitForSalesforceReport() {
  try {
    ...
    if (!widget.classList.contains('widgetReady') || !widget.classList.contains('finalState')) {
      return false;
    }
 ...
 *   - table.data-grid-fixed-column-table (pinned: End Date, Task Name)
 *   - table.data-grid-full-table (all columns)
 *
 * Cell values via div.wave-table-cell-text[aria-label]:
 *   Format: "Column Header: Value"
 *
 * Rows grouped by End Date / Task Name via rowspan.
 *
 * Falls back to legacy td[data-fixed-column] extraction if Wave
 * selectors don't match (backwards compatibility).
 */
  ...
  try {
    ...
    if (widget && widget.classList.contains('widgetReady') && widget.classList.contains('finalState')) {
      ...
      if (table) {
        ...
        // Known field names for compound header detection
        ...
        rows.forEach(function (row) {
          ...
          gridCells.forEach(function (cell, cellIndex) {
            ...
            // Handle compound headers like "Project (Rollup): Project Name: Value"
            if (ariaLabel && ariaLabel.includes(':')) {
              var parts = ariaLabel.split(': ');
              var columnHeader = parts[0];
              var value = parts.slice(1).join(': ');

              // Check if parts[1] looks like a field name (compound header)
              if (parts.length >= 3) {
...
function extractOutlookData() {
  try {
    ...
    return { needsLogin: false, data: labels };
  } catch (e) {
...
function clickCloudCoachTicketsSidebar() {
  try {
    ...
    for (var j = 0; j < clickables.length; j++) {
      var text = (clickables[j].textContent || '').trim();
      // Match exact "Tickets" or text containing "Tickets" with count badge
...
function isTicketsTableReady() {
  try {
    // Validate we're in a frame that might have the data
    ...
    if (table) {
      ...
      if (tbody) {
        ...
        // Require at least one data row, not just existence
        if (rows.length > 0) {
    ...
    // Look for tables with semantic column headers matching Tickets structure
    var tables = document.querySelectorAll('table');
    for (var t = 0; t < tables.length; t++) {
      ...
      // Check for Cloud Coach column patterns
 ...
 *
 * Uses SLDS output components for cell extraction:
 * - Cell 2: Due Date (slds-output-date span[title])
 * - Cell 3: Name (slds-output-string div[title] or div.slds-output-string)
 * - Cell 4: Project (slds-lookup-output a)
 * - Cell 5: Description (slds-rich-text-editor__output[title])
 * - Cell 6: Priority (slds-output-picklist span[title])
 * - Cell 7: Created Date (slds-output-datetime span[title])
 * - Cell 8: Completed (slds-output-boolean input[type="checkbox"])
 * - Cell 0: Status (warning/error icons)
 *
 * Falls back to generic table extraction if cc-sobject-table not found.
 */
    ...
    if (results.length === 0) {
      ...
      if (isCardFallbackFrame) {
        ...
        cards.forEach(function (el) {
          ...
          // Skip if matches known navigation patterns
          if (knownNavPatterns.some(function(p) { return textLower.includes(p); })) {
            return;
          }

          // Check for garbage patterns like "DashboardsDashboards"
          if (t.includes('Dashboards') && textLower.split('dashboards').length > 2) {
            return;
          }

          // Reasonable length check (not too short, not too long)
 ...
 *
 * Collects ALL frame results and intelligently chooses which one to return.
 * Prefers: vf.force.com/lightningReportApp frames > more data > no garbage patterns
 */
async function runExtraction(tabId, extractionFunc, allFrames = false) {
  ...
  const errors = [];
  const allDiag = [];
  const qualifiedResults = [];  // Collect all viable results

  // Collect all results instead of returning on first match
  ...
  // Prioritize results before returning
...
/**
 * Poll a readiness function until it returns true in at least one frame.
 * @param {number} tabId
 * @param {Function} readinessFunc - Self-contained function returning boolean
 * @param {boolean} allFrames
 * @param {number} timeoutMs - Max polling time (default 30s)
 * @param {number} intervalMs - Polling interval (default 1s)
 * @returns {Promise<boolean>}
 */
async function pollReadiness(tabId, readinessFunc, allFrames, timeoutMs = 30000, intervalMs = 1000) {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      ...
      // Script injection failed — wait and retry
    }
 ...
 *
 * Supports preExtractFunc as either a single function or array of sequential steps.
...
async function importSource(key, urlPrefix, fullUrl, extractFunc, allFrames, preExtractFunc, readinessFunc) {
  ...
  // Support array of pre-extraction functions for multi-step flows
  if (preExtractFunc) {
    ...
    for (var stepIdx = 0; stepIdx < preExtractFuncs.length; stepIdx++) {
      ...
      for (let preAttempt = 0; preAttempt < 3; preAttempt++) {
        try {
          ...
          if (preClickResult) {
            // Better inter-step delay for Cloud Coach
      ...
      if (!preClickResult && key === 'cloudcoach') {
        // Log diagnostic info for failed steps
  ...
  if (readinessFunc) {
    ...
    if (!isReady) {
      // Log warning and handle differently based on source
 ...
 *
 * Cloud Coach now uses [navigateToCloudCoachTickets, clickCloudCoachTicketsSidebar]
  ...
  if (sources.includes('salesforce')) {
    promises.push(
      importSource(
        ...
        true,                        // allFrames
        null,                        // no preExtract
        waitForSalesforceReport      // readiness polling
  ...
  if (sources.includes('cloudcoach')) {
    promises.push(
      importSource(
        ...
        true,                                                          // allFrames
        // Two-step navigation array instead of single function
        [navigateToCloudCoachTickets, clickCloudCoachTicketsSidebar],
        // New readiness function that validates Tickets table
        isTicketsTableReady
...
/** Check login status for all sources without extracting. */
async function checkSources() {
  const statuses = {};

  for (const [key, url] of Object.entries(SOURCE_URLS)) {
    const prefix = key === 'outlook' ? 'https://outlook.office' : url.split('/lightning/')[0] + '/lightning/';
    const tab = await findTab(prefix);
    if (tab) {
      statuses[key] = { open: true, tabId: tab.id };
    } else {
      statuses[key] = { open: false };
    }
  }
...
// ── Inject content bridge into already-open localhost tabs ─────

async function injectContentBridgeIntoExistingTabs() {
  try {
    const tabs = await chrome.tabs.query({
    ...
    for (const tab of tabs) {
      ...
      } catch (e) { /* not scriptable */ }
    }
  } catch (e) { /* query failed */ }
}

chrome.runtime.onInstalled.addListener(() => {
  injectContentBridgeIntoExistingTabs();
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (message.action) {
    case 'wmp-import-all':
      importAll(message.options)
        .then((results) => sendResponse({ success: true, results }))
        .catch((err) => sendResponse({ success: false, error: err.message }));
      return true;

    case 'wmp-import-source':
      {
        const { key } = message;
        const config = {
          salesforce: {
        ...
        if (!config) {
          sendResponse({ success: false, error: 'Unknown source: ' + key });
          return;
        }

        importSource(key, config.prefix, config.url, config.func, config.allFrames, config.preExtract, config.readiness)
          .then((result) => sendResponse({ success: true, result }))
          .catch((err) => sendResponse({ success: false, error: err.message }));
        return true;
      }

    case 'wmp-check-sources':
      checkSources()
